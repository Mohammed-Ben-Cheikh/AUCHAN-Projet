    ...
    function increment(event, element, quantite, price) {
        ...
        temp.forEach((data) => {
            if(data.title == element.title) {
                ...
                price.dataset.price = parseFloat(data.price).toFixed(2);
...
function CartVerification(data) {
    ...
    function addProduct(element) {
        ...
        checkbox.className = 'checkbox h-5 w-5';
    ...
    function countTotalSelection(event) {
        let checkboxes = document.querySelectorAll('.checkbox');
        let totalSelected = document.querySelector('.totalSelected');
        let total = 0;
    
        checkboxes.forEach(checkbox => {
            if (checkbox.checked) {
                let element = checkbox.parentElement.parentElement.parentElement;
                let price = element.querySelector('.price');
